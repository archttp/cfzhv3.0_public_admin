<template>
  <a-form ref="formRef" :model="formData" style="margin-top: 32px">
    ...
    >
    <a-form-item
      field="identity_info.id_doc_type"
      label="证件类型"
      validate-trigger="input"
      required
    >
      <a-select
        v-model="formData.identity_info.id_doc_type"
        placeholder="请选择"
        allow-clear
      >
    ...
    >
      <a-select
        ...
        allow-clear
      >
        <a-option value="LEGAL"
          >法人</a-option
        >
        <a-option value="SUPER"
          >经办人</a-option
        >
      </a-select>
    </a-form-item>
...
<script lang="ts" setup>
  ...
  const onSuccess1 = (fileRes: any) => {
    formData.value.business_license_info.license_copy =
      fileRes.response.data.id;
  };
  const onSuccess2 = (fileRes: any) => {
    formData.value.identity_info.id_card_info.id_card_national =
      fileRes.response.data.id;
  };

  onMounted(() => {
    if (localStorage.getItem('enterprise-Subject')) {
      // @ts-ignore
      formData.value = JSON.parse(localStorage.getItem('enterprise-Subject'));
    }
  });
  const onNext = () => {
