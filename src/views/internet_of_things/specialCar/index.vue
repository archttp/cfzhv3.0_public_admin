  ...
  <div class="container">
    <Breadcrumb
      :items="[
        ...
        'menu.pay.internetofthings.specialcar',
      ]"
    />
    <a-card
      class="general-card"
      :title="$t('menu.pay.internetofthings.specialcar')"
    >
      <a-row>
        <a-col :flex="1">
          <a-form
...
<script lang="ts" setup>
  ...
  const onDelete = async (id: string) => {
    await specialCarDelete({ params: { id } });
    fetchData();
  };
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await specialCarGet({
      params: checkParams({
