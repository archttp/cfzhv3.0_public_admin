        ...
        <a-space direction="vertical" :size="16">
          <a-card class="general-card">
            ...
            <a-row :gutter="80">
              <a-col :span="8">
                <a-form-item label="充电站" field="chargingStationId" required>
                  <a-select :loading="selectLoading" placeholder="请输入" allow-search allow-clear @search="handleSelectSearch"
                    :filter-option="false" v-model="formData.chargingStationId">
              ...
              <a-col :span="8">
                <a-form-item label="卡编号" field="cardNo" validate-trigger="input" required>
                  <a-input v-model="formData.cardNo" placeholder="请输入" allow-clear></a-input>
                </a-form-item>
              </a-col>
              <a-col :span="8">
...
import { addChargingStoredCard, updateChargingStoredCard, cfChargingStationSearch } from '@/api/internetOfThings';
...
const onSubmitClick = () => {
  ...
  formRef.value.validate(async (valid: any) => {
    ...
    if (formData.value.id) {
      const { code } = await updateChargingStoredCard(newForm);
      if (code === 10002) {
        formRef.value.resetFields();
        visible.value = false;
      ...
      const { code } = await addChargingStoredCard(newForm);
      if (code === 10002) {
        formRef.value.resetFields();
        visible.value = false;
