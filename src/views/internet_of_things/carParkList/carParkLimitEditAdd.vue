      ...
      <a-form ref="formRef" layout="vertical" :model="formData">
        <a-space direction="vertical" :size="16">
            ...
            <a-row :gutter="80">
              ...
              <a-col :span="8">
                <a-form-item label="收费模式" field="billingModel" required>
                  <a-select v-model="formData.billingModel" placeholder="请选择" allow-clear>
                    ...
                    <a-option value="dynamic_time_upper_limit">每天指定最高费用</a-option>
                    <a-option value="static_time_upper_limit">限时指定最高费用</a-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :span="8">
              ...
              <a-col :span="8">
                <a-form-item
                  ...
                  field="feeUpperLimit"
                  required
                  tooltip="必须填写，不能填0，例如每天或者每24小时最高收费是多少"
                >
                  <a-input
          ...
          <a-card class="general-card">
            ...
            <a-row :gutter="80">
              <a-col :span="8">
                <a-form-item label="是否自动开闸(入场)" field="autoOpenDoor">
              ...
              <a-col :span="8">
                <a-form-item label="是否禁止出场" field="forbidOut">
                  <a-select placeholder="请选择" allow-clear v-model="formData.forbidOut">
                    <a-option :value="0">否</a-option>
                    <a-option :value="1">是</a-option>
                  </a-select>
                </a-form-item>
              ...
              <a-col :span="8">
                <a-form-item label="免费放行(出场)" field="freeRelease">
              ...
              <a-col :span="8">
                <a-form-item
                  label="起步价模式"
                  field="startingPriceMode"
                  validate-trigger="input"
                >
                  <a-select placeholder="请选择" allow-clear v-model="formData.startingPriceMode">
                    <a-option :value="0">每次都收</a-option>
                    <a-option :value="1">提前缴费只收一次</a-option>
                    <a-option :value="2">每个周期都收</a-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :span="8">
              ...
              <a-col :span="8">
                <a-form-item label="追缴期限(天)" field="recoveryDeadline">
                  <a-input v-model="formData.recoveryDeadline" placeholder="请输入" allow-clear />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item label="车辆过期变临时车限制时间(天)" field="expireToTemporaryLimit">
                  <a-input v-model="formData.expireToTemporaryLimit" placeholder="请输入" allow-clear />
                </a-form-item>
              </a-col>
            </a-row>
          </a-card>
...
const formData = ref({
  ...
  template: 0,
  unitDuration: 60,
  recoveryDeadline: 365,
  expireToTemporaryLimit: 60,
});
...
const handleEdit = (row: any = null) => {
  ...
  formData.value.recoveryDeadline = row.recoveryDeadline/86400000;
  formData.value.expireToTemporaryLimit = row.expireToTemporaryLimit/86400000;
  // @ts-ignore
  formData.value.leaveTimeLimit = row.leaveTimeLimit/60000;
  // @ts-ignore
  formData.value.unitDuration = row.unitDuration/60000;
  getCarType();
  visible.value = true;
};
...
const onSubmitClick = () => {
  ...
  formRef.value.validate(async (valid: any) => {
    ...
    newFormData.recoveryDeadline = formData.value.recoveryDeadline * 86400000;
    newFormData.expireToTemporaryLimit = formData.value.expireToTemporaryLimit * 86400000;
    // @ts-ignore
    newFormData.leaveTimeLimit = formData.value.leaveTimeLimit * 60000;
    // @ts-ignore
    newFormData.unitDuration = formData.value.unitDuration * 60000;
    // @ts-ignore
