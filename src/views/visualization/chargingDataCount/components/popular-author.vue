<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '14px' }">
      <template #title> 异常充电排行(本月) </template>
      <!-- <template #extra>
      ...
      >
        <template #columns>
          <a-row>
            <a-col :flex="8">
              <a-table-column title="充电站名称" data-index="title">
              </a-table-column>
            </a-col>
            <a-col :flex="4">
              <a-table-column
                title="异常次数"
                data-index="abnormalCounts"
              >
              </a-table-column>
            </a-col>
          </a-row>
...
<script lang="ts" setup>
  ...
  import {
    queryPopularAuthor,
    PopularAuthorRes,
    getDailyChargingStatistcsListByQuery,
