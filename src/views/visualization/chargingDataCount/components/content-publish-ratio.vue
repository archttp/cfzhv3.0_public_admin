<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '14px' }">
      <template #title> 充电站数量变动(月数据) </template>
      <!-- <template #extra>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      for (const i of data) {
        ...
        blueChartsData.value.push(i.chargingStationCounts);
        greenChartsData.value.push(i.carChargingStationCounts);
        yellowChartsData.value.push(i.bicycleChargingStationCounts);
      }
      setLoading(false);
    } catch (err) {
    } finally {
