<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '16px' }">
      <template #title>
        {{ $t('dataAnalysis.contentVillagePeriodAnalysis') }}
      </template>
...
<script lang="ts" setup>
  ...
  const tenantResidentCounts = ref<number[]>([]);
  const officeResidentCounts = ref<number[]>([]);

  const inOutRecordsCounts = ref<number[]>([]);
  const visitingRecordsCounts = ref<number[]>([]);
  const roughcastHouse = ref<number[]>([]);
  const rentalHousing = ref<number[]>([]);
  const onSaleRoom = ref<number[]>([]);
  const vacantHouse = ref<number[]>([]);
  ...
  const { chartOption } = useChartOption((isDark) => {
    return {
      ...
      series: [
        {
          name: '租客数',
          data: tenantResidentCounts.value,
          type: 'line',
          smooth: true,
          showSymbol: false,
        ...
        {
          name: '办公人员数',
          data: officeResidentCounts.value,
          type: 'line',
          smooth: true,
          showSymbol: false,
        ...
        {
          name: '预约到访数',
          data: visitingRecordsCounts.value,
          type: 'line',
          smooth: true,
          showSymbol: false,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      for (const i of data) {
        ...
        tenantResidentCounts.value.push(i.tenantResidentCounts);
        officeResidentCounts.value.push(i.officeResidentCounts);
        inOutRecordsCounts.value.push(i.inOutRecordsCounts);
        visitingRecordsCounts.value.push(
          i.visitingRecordsCounts
        );
        roughcastHouse.value.push(
          i.roughcastHouse
        );
        rentalHousing.value.push(i.rentalHousing);
        onSaleRoom.value.push(i.onSaleRoom);
        vacantHouse.value.push(i.vacantHouse);
      }
    } catch (err) {
