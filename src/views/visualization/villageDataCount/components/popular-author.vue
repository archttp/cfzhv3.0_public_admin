<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '14px' }">
      <template #title> 社区居民数量排行(本月) </template>
      <!-- <template #extra>
      ...
      >
        <template #columns>
          <a-row>
            <a-col :flex="8">
              <a-table-column title="社区名称" data-index="title">
              </a-table-column>
            </a-col>
            <a-col :flex="4">
              <a-table-column
                title="居民数量"
                data-index="residentCounts"
              >
              </a-table-column>
            </a-col>
          </a-row>
...
<script lang="ts" setup>
  ...
  import {
    getDailyVillageStatistcsListByQuery,
